...
Therefore, the cardinality of $\mathcal{P}_I$, which is the set of all transformation matrices that are compatible with a given \textit{a priori} information $I$, measures exactly the degree of identification one is able to achieve \textit{relative} to her \textit{a priori} information $I$. That is, identification might be quite expensive and $I$ is the price that one must pay for it. But do we always need to pay a high price in order to buy identification?